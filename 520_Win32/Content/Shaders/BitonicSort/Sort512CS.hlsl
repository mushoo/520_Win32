...
//--------------------------------------------------------------------------------------
groupshared AABBNode g_LDS[SORT_SIZE];
...
{
	int GlobalBaseIndex = (Gid.x * SORT_SIZE) + GTid.x + g_TreeOffset;
	int LocalBaseIndex = GI;

	int numElementsInThreadGroup = min(SORT_SIZE, g_NumElements - (Gid.x * SORT_SIZE));
	...
	for (unsigned int nMergeSize = 2; nMergeSize <= SORT_SIZE; nMergeSize = nMergeSize * 2)
	{
		for (int nMergeSubSize = nMergeSize >> 1; nMergeSubSize>0; nMergeSubSize = nMergeSubSize >> 1)
		{
			[unroll]for (i = 0; i<ITERATIONS; ++i)
			{
				...
				{
					AABBNode a = g_LDS[index];
					AABBNode b = g_LDS[nSwapElem];
